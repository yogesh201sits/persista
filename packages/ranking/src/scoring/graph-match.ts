...
export function findGraphMatches(
  ...
  const content = vectorResult.metadata!.content.toLowerCase();

  const matches: GraphMatch[] = [];

  if (content.includes(graphResult.entity.name.toLowerCase())) {
    matches.push({
      entityId: graphResult.entity.id,
      depth: 0,
      confidence: 1,
    });
  }
  ...
  for (const item of graphResult.relationships) {
    if (content.includes(item.entity.name.toLowerCase())) {
      matches.push({
