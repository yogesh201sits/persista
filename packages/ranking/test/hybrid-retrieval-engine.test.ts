...
describe("DefaultHybridRetrievalEngine", () => {
  ...
  function createEngine({
    ...
    const vectorSearch = mock(
      async () => vectorResults,
    );

    const graphSearch = mock(
      async () => graphResult,
    );
  ...
  test("should perform graph search using the first extracted entity", async () => {
    ...
    await engine.search(
      "What does Yogesh use for backend?",
    );
  ...
  test("should include graph entity in results", async () => {
    ...
    expect(
      result.results[0].relationships,
    ).toEqual(
