...
memories.post("/", validateBody(rememberRequestSchema), async (c) => {
  ...
  await memoryManager.remember(body.conversation);

  await graphMemory.remember(body.conversation);
...
memories.put("/:id", validateBody(updateMemoryRequestSchema), async (c) => {
  const id = c.req.param("id");
